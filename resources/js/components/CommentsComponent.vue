<template>
    <div class="col comments-card_single-comments__wrapper border border-dark shadow-sm" v-if="comment.review_id === $props.reviewId">
        <div class="card-body comment-card__single-card" >
            {{this.$props.comment.comments}}
            <router-link class="comment-card__single-card__router-link" :to="`/comments/${$props.comment.id}/edit`">🖉</router-link>
            <form  class="review-card__single-review_remove-comment right-side" v-on:submit.prevent="deleteComments($props.comment.id)">
                <input :disabled="$store.state.isDisable" type="submit" class="remove-comment__button-remove" name="btnDelete" value="⨯">
            </form>
        </div>
    </div>             
</template>
<script>
export default {
    methods: {
        //deleteting Comments
        deleteComments(id){  
            this.$store.commit('disableButton');
            axios.delete(`/api/comments/${id}`)
             .then(()=> window.location.href = '/reviews');
        }
    },
    props: {
        comment: {
            type: Object
        },
        reviewId: {
            type: Number
        }
    }
};
</script>
